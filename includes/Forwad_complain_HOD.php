<?php
        ...
        $sql = "SELECT * from complain c inner join user_details ud on c.uid=ud.uid where ud.uid=".$_SESSION['USER'];
        ...
        $sql = "SELECT * from complain_log cl inner join complain c on cl.c_id = c.id inner join user u on u.uid=c.uid inner join user_details ud on u.uid=ud.uid where cl.status='0' AND dept=".$dept;
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
            ?>
                    <div class="card horizontal complain_detail">
                        <div class="card-image complain_img">
                            <img class="materialboxed" src="uploads/<?php echo $row['img_url'];?>" height="260" width="200px">
                        </div>
                                                ...
                                                <select name="forward_select">
                                                    ...
                                                    <?php
                                                        if($row['level']==2){
                                                            ...
                                                            echo "<option value=".$row['uid'].">Principle</option>";
                                                        }else{
                                                            ...
                                                            WHERE col.c_id=c.id AND c.complain_commitee=cl.c_id AND ci.c_id=c.complain_commitee AND ci.uid=u.uid AND c.complain_commitee=".$row['complain_commitee'];
